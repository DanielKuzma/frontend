...
const Devices = () => {
    ...
    const handleAddDevice = async (e) => {
        ...
        if (nameExists) {
            showNotification('Błąd: Urządzenie o takiej nazwie już istnieje w budynku! Użyj unikalnej nazwy.', 'warning');
            return; 
        }
        ...
        try {
            ...
            showNotification('Wystąpił błąd podczas dodawania urządzenia.', 'danger');
        }
    };

    // WYKONANIE USUNIĘCIA PO POTWIERDZENIU
    ...
    const handleSaveRule = async (e) => {
        ...
        try {
            ...
            showNotification('Błąd serwera podczas dodawania reguły.', 'danger');
        }
    };
        ...
        <div className="mt-4 container-main-view">
            ...
            <div className="main-card-container shadow border-0">
                <div className="d-flex justify-content-between align-items-center mb-4">
                    ...
                    {canAddDevice && (
                        <Button variant="primary" onClick={() => setShowModal(true)}>+ Dodaj Urządzenie</Button>
                    )}
                </div>

                {devices.length === 0 ? (
                    <p className="text-center py-5" style={{ color: 'var(--text-sub)' }}>Brak urządzeń w tym pomieszczeniu.</p>
                ) : (
                                ...
                                >
                                    <Card.Body className="d-flex flex-column p-4">
                                        <div className="d-flex justify-content-between align-items-start mb-3">
                                            <Card.Title className="fw-bold m-0" style={{ color: 'var(--accent-cyan)', fontSize: '1.25rem' }}>{device.name}</Card.Title>
                                            <Badge bg={getStatusBadgeVariant(device.deviceStatus)} className="px-2 py-1">{device.deviceStatus || 'BŁĄD DANYCH'}</Badge>
                                        </div>
                                        ...
                                        <div className="mt-auto d-flex justify-content-between align-items-center gap-2">
                                            <div className="d-flex gap-2 flex-grow-1">
                                                <Button 
                                                    ...
                                                    size="sm" className="fw-bold flex-grow-1"
                                                    onClick={() => toggleStatus(device.id, device.deviceStatus)}
                                                    disabled={device.deviceStatus === 'OFFLINE'} 
                                                >{device.deviceStatus === 'ON' ? 'Wyłącz' : 'Włącz'}</Button>
                                                {canAddDevice && (
                                                    <Button variant="outline-info" size="sm" onClick={() => handleOpenRuleCreator(device)}>Automatyzuj</Button>
                                                )}
                                            </div>
                                            {canDeleteDevice && (
                                                <Button variant="outline-danger" size="sm" onClick={() => setDeleteConfirmInfo({ show: true, id: device.id })}>Usuń</Button>
                                            )}
                                        </div>
                                    </Card.Body>
                                </Card>
                    ...
                    <Modal.Body className="p-4">
                        <Form onSubmit={handleSaveRule}>
                            <Form.Group className="mb-3">
                                ...
                                <Form.Control type="text" required placeholder="np. Wyłącz klimatyzację w nocy" value={newRule.name} onChange={(e) => setNewRule({ ...newRule, name: e.target.value })} style={{ backgroundColor: 'var(--input-bg)', color: 'var(--text-main)' }} />
                            </Form.Group>
                            <Form.Group className="mb-3">
                                <Form.Label>Wyzwalacz (Czujnik)</Form.Label>
                                <Form.Select required value={newRule.sensorId} onChange={(e) => setNewRule({ ...newRule, sensorId: e.target.value })} style={{ backgroundColor: 'var(--input-bg)', color: 'var(--text-main)' }} >
                                    ...
                                    {availableSensorsForSelectedDevice.length === 0 ? (
                                        ...
                                        availableSensorsForSelectedDevice.map(sensor => (
                                            <option key={sensor.id} value={sensor.id}>{sensor.name} {sensor.unit ? `(${sensor.unit})` : ''}</option>
                                        ))
                                    )}
                                </Form.Select>
                            </Form.Group>
                            <Row className="mb-3">
                                <Col>
                                    <Form.Group>
                                        ...
                                        <Form.Select value={newRule.operator} onChange={(e) => setNewRule({ ...newRule, operator: e.target.value })} style={{ backgroundColor: 'var(--input-bg)', color: 'var(--text-main)' }}>
                                ...
                                <Col>
                                    <Form.Group>
                                        ...
                                        <Form.Control type="number" step="0.1" required placeholder="np. 25" value={newRule.threshold} onChange={(e) => setNewRule({ ...newRule, threshold: e.target.value })} style={{ backgroundColor: 'var(--input-bg)', color: 'var(--text-main)' }} />
                                    </Form.Group>
                                </Col>
                            </Row>
                            <Form.Group className="mb-4">
                                <Form.Label>Wtedy zmień status urządzenia na:</Form.Label>
                                <Form.Select value={newRule.targetStatus} onChange={(e) => setNewRule({ ...newRule, targetStatus: e.target.value })} style={{ backgroundColor: 'var(--input-bg)', color: 'var(--text-main)' }}>
                                    <option value="ON">Włączone (ON)</option>
                                    <option value="OFF">Wyłączone (OFF)</option>
                                </Form.Select>
                            </Form.Group>
                            <div className="d-flex justify-content-end gap-2">
                                ...
                                <Button variant="success" type="submit" disabled={availableSensorsForSelectedDevice.length === 0 || !newRule.name || !newRule.sensorId || !newRule.threshold} className="fw-bold px-4">Zapisz i aktywuj</Button>
                            </div>
                        </Form>
                    </Modal.Body>
                </div>
            ...
            {/* MODAL POTWIERDZENIA USUNIĘCIA URZĄDZENIA */}
            <Modal show={deleteConfirmInfo.show} onHide={() => setDeleteConfirmInfo({ show: false, id: null })} centered size="sm">
                <div style={{ backgroundColor: 'var(--card-bg)', color: 'var(--text-main)', borderRadius: '12px', border: '1px solid var(--accent-hover)' }}>
                    <Modal.Body className="p-4 text-center">
                        ...
                        <p className="text-muted" style={{ fontSize: '0.95rem' }}>Czy na pewno chcesz usunąć to urządzenie?</p>
                        <div className="d-flex justify-content-center gap-2 mt-4">
                            <Button variant="secondary" onClick={() => setDeleteConfirmInfo({ show: false, id: null })}>Anuluj</Button>
                            <Button variant="danger" className="fw-bold" onClick={executeDeleteDevice}>Tak, usuń</Button>
                        </div>
                    </Modal.Body>
                </div>
            </Modal>
